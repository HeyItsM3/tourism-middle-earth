			...
			<tbody>
				<c:forEach items="${productos}" var="producto">
					<tr>
						...
						<td><c:if test="${producto.hayCupo()}">
								<c:out value="Hay cupo"></c:out>
							</c:if>
							<c:choose>
								<c:when test="${!producto.hayCupo()}">
									<c:out value="No hay cupo"></c:out>
								</c:when>
							</c:choose></td>
						...
						<td><c:if test="${user.admin}">
								...
								<a href="/torre/deleteAtracc.do?id=${producto.id}"
							...
							<c:choose>
								<c:when
									...
									<a
										href="/torre/buy.do?id=${producto.id}&esPromo=${producto.esPromo()}"><i
										class="fas fa-shopping-cart"></i></a>
								</c:when>
								<c:when test="${!user.productoNoComprado(producto)}">
