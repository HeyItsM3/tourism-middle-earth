...
public class AtraccionDAOImpl implements AtraccionDAO {
	...
	private Atraccion toAtraccion(ResultSet resultado) {
		try {
			return new Atraccion(resultado.getString(2), resultado.getInt(3), resultado.getInt(4),
					TipoDeAtraccion.valueOf(resultado.getString(5)), resultado.getInt(1), resultado.getInt(6), resultado.getString(8));
	...
	public int insert(Atraccion atraccion) {
		try {
			String sql = "INSERT INTO ATRACCIONES (NOMBRE, COSTO, DURACION, TIPO_ATRACCION, CUPO, IMG_URL) VALUES (?, ?, ?, ?, ?, ?)";
			Connection conn = ConnectionBBDD.getConnection();
			...
			statement.setString(i++, atraccion.getImag_u());
